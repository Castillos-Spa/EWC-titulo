...
export const routeAccess: Record<RouteId, Role[]> = {
  ...
  "buk-users": powerRoles,
};

export function isRouteAllowedByUser(
  routeId: RouteId,
  user: User | null
): boolean {
...
export function getPreferredRoute(user: User | null): string {
  try {
    ...
    if (
      ...
      lastPage &&
      lastPage.startsWith("/") &&
      !["/login", "/forgot-password"].includes(lastPage)
    ) {
      // Intentar mapear lastPage a un RouteId para validar acceso; si no se reconoce, usar tal cual.
      const entry = Object.entries(routeIdToPath).find(
        ([, p]) => p === lastPage
      ) as [RouteId, string] | undefined;
    ...
    const firstAllowed = (Object.keys(routeIdToPath) as RouteId[]).find((rid) =>
      isRouteAllowedByUser(rid, user)
    );
