...
import StandardVehicleDirectory from '../components/StandardVehicleDirectory';
import StandardVehicleModal from '../components/StandardVehicleModal';
import { StandardFleetProvider, useStandardFleet } from '../context/StandardFleetContext';

function Inner() {
  const { openCreate, setSearch } = useStandardFleet();
  useEffect(() => {
  ...
  return (
    <div className="space-y-10 text-slate-800 dark:text-slate-100">
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-8">
          <div className="max-w-2xl space-y-3">
            <span className="inline-flex items-center gap-2 rounded-full border border-white/60 bg-white/80 px-3 py-1 text-xs font-semibold uppercase tracking-[0.32em] text-slate-600 shadow-sm dark:border-white/10 dark:bg-white/5 dark:text-blue-100">
              ...
              <span>Flota — Estándar</span>
            </span>
            <h1 className="text-3xl font-semibold tracking-tight text-slate-900 dark:text-white">Registro de Flota (Mock)</h1>
            <p className="text-sm text-slate-600 dark:text-blue-100/80">Versión estandarizada para demo: multiempresa, códigos únicos por empresa y campos extendidos.</p>
          </div>
          <div className="flex items-center gap-3">
            <a href="/flota" className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white/80 px-5 py-3 text-sm font-semibold text-slate-600 shadow-sm transition hover:-translate-y-0.5 hover:border-sky-300 hover:text-slate-800 dark:border-white/10 dark:bg-white/10 dark:text-blue-100">Volver a versión actual</a>
            <button
              type="button"
              onClick={openCreate}
      ...
      <StandardVehicleDirectory />
      <StandardVehicleModal />
    </div>
  );
}

export default function StandardFleetPage() {
  return (
    <StandardFleetProvider>
      <Inner />
    </StandardFleetProvider>
  );
}
