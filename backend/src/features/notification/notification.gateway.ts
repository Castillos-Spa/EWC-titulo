  ...
  async handleConnection(client: Socket) {
    ...
    try {
      ...
      if (userId && Number.isFinite(userId) && userId > 0) {
        ...
        const user = await this.notificationService.getUserRolesAndAreas(userId);
        if (user) {
        ...
        const notifications = await this.notificationService.findAllForUser(userId, new PaginationQueryDto());
